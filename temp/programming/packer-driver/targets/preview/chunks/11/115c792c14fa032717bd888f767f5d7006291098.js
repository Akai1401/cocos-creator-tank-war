      ...
      _export("roomCtrl", roomCtrl = (_dec = ccclass("roomCtrl"), _dec2 = property({
        ...
        onLoad() {
          ...
          window.socketManager.onmessage = event => {
            var data = JSON.parse(event.data); // on startGame event
        ...
        removeUserFromList(data) {
          this.userListContainer.removeAllChildren();
          var newArr = window.userListArr.filter(item => item.id !== (data == null ? void 0 : data.id));
        ...
        renderUserList() {
          window.userListArr.forEach(item => {
            ...
            var userNode = instantiate(this.tank);
            ...
            var userCount = this.userListContainer.children.length;
            var row = Math.floor(userCount / 4);
            var col = userCount % 4;
            var xOffset = 150;
            var yOffset = 150;
            var newPosition = new Vec3(-220 + col * xOffset, 100 + -row * yOffset, 0);
...
//# sourceMappingURL=115c792c14fa032717bd888f767f5d7006291098.js.map